...
func init() {
	android.RegisterModuleType("octavi_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = octaviExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = octaviExpandVariables(ctx, dep_file)
	...
	exCmd := octaviExpandVariables(ctx, String(g.properties.Cmd))
