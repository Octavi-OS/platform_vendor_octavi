...
function breakfast()
{
    target=$1
    local variant=$2
    OCTAVI_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    for f in `/bin/ls vendor/octavi/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function deleteOTA() {
    ...
    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        ...
        --request DELETE \
        https://api.octavi.org/api/ota/builds/$1/;
}
...
function pushOTA() {
    ...
    device=$(grep ro\.octavi\.device $OUT/system/build.prop | cut -d= -f2);
    file=$(ls -t ${OUT}/octavi_$device-10* | sed -n 2p);
    md5=$(md5sum $file | awk '{ print $1 }');
    build_type=$(echo $BUILD_TYPE | tr '[:upper:]' '[:lower:]');
    size=$(stat -c%s $file);
    version=$(grep ro\.octavi\.status $OUT/system/build.prop | cut -d= -f2);
    if [ -z $version ]; then
        version=$(grep ro\.octavi\.status $OUT/vendor/build.prop | cut -d= -f2)
    fi
    notes=""
    ...
    url="https://sourceforge.net/projects/Octavi-OS/files/$device/${file##*/}";
    if [[ "${TEST_BUILD}" == true ]]; then
        url="https://sourceforge.net/projects/Octavi-OS/files/$device/cucked/${file##*/}";
    fi

    data="{\"build_date\":\"$build_date\", \"device\":\"$device\",\"filename\":\"${file##*/}\",\"md5\":\"$md5\",\"build_type\":\"$build_type\",\"size\":\"$size\",\"url\":\"$url\",\"version\":\"$version\",\"notes\":\"$notes\"}";

    curl --header "Authorization: Token $OTA_API_USER_TOKEN" \
        --header "Content-Type: application/json" \
        --request POST \
        --data "$data" \
        https://api.octavi.org/api/ota/builds/;
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/octavi/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
