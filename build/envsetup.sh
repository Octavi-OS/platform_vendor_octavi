function __print_octavi_functions_help() {
cat <<EOF
Additional OctaviOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- octavigerrit:    A Git wrapper that fetches/pushes patch from/to OctaviOS Gerrit Review.
- octavirebase:    Rebase a Gerrit change and push it again.
- octaviremote:    Add git remote for OctaviOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for OctaviOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Octavi model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch octavi_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/octavi-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.octavi.device | grep -q "$OCTAVI_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $OCTAVI_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function octaviremote()
{
    ...
    git remote rm octavi 2> /dev/null
    ...
    local OCTAVI_USER=$(git config --get review.review.octavios.org.username)
    if [ -z "$OCTAVI_USER" ]
    then
        git remote add octavi ssh://review.octavios.org:29418/$PFX$PROJECT
    else
        git remote add octavi ssh://$OCTAVI_USER@review.octavios.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'octavi' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Octavi-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.octavi.device | grep -q "$OCTAVI_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        octaviremote
        git push octavi HEAD:refs/heads/'$1'
    fi
    '
}

function octavigerrit() {
    ...
    local user=`git config --get review.review.octavios.org.username`
...
function octavirebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "OctaviOS Gerrit Rebase Usage: "
        echo "      octavirebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.octavios.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.octavi.device | grep -q "$OCTAVI_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/octavi/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $OCTAVI_FIXUP_COMMON_OUT ]; then
