...
function setup_vendor() {
    ...
    export OCTAVI_ROOT="$3"
    if [ ! -d "$OCTAVI_ROOT" ]; then
        echo "\$OCTAVI_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$OCTAVI_ROOT/$OUTDIR" ]; then
        mkdir -p "$OCTAVI_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$OCTAVI_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$OCTAVI_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$OCTAVI_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$OCTAVI_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR OctaviOS\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR OctaviOS\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR OctaviOS\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) 2017-$YEAR OctaviOS\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR OctaviOS\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR OctaviOS\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$OCTAVI_ROOT"/prebuilts/tools-octavi/common/smali/baksmali.jar
        export SMALIJAR="$OCTAVI_ROOT"/prebuilts/tools-octavi/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$OCTAVI_ROOT"/prebuilts/tools-octavi/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$OCTAVI_ROOT"/prebuilts/tools-octavi/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$OCTAVI_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$OCTAVI_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$OCTAVI_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$OCTAVI_ROOT"/vendor/octavi/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$OCTAVI_ROOT"/"$OUTDIR"/radio
