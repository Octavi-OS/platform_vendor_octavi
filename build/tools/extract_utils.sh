...
function setup_vendor() {
    ...
    export DERP_ROOT="$3"
    if [ ! -d "$DERP_ROOT" ]; then
        echo "\$DERP_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$DERP_ROOT/$OUTDIR" ]; then
        mkdir -p "$DERP_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$DERP_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$DERP_ROOT"/"$OUTDIR"/Android.mk
    export ANDROIDBP="$DERP_ROOT"/"$OUTDIR"/Android.bp
    export BOARDMK="$DERP_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "$COMMENT_PREFIX Copyright (C) $YEAR DerpFest\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "$COMMENT_PREFIX Copyright (C) 2017-$YEAR DerpFest\n" >> $1
        else
            printf "$COMMENT_PREFIX Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR DerpFest\n" >> $1
        fi
    else
        printf "$COMMENT_PREFIX Copyright (C) $YEAR DerpFest\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$DERP_ROOT"/vendor/derp/build/tools/smali/baksmali.jar
        export SMALIJAR="$DERP_ROOT"/vendor/derp/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$DERP_ROOT"/vendor/derp/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$DERP_ROOT"/vendor/derp/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$DERP_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$DERP_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$DERP_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$DERP_ROOT"/vendor/derp/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$DERP_ROOT"/"$OUTDIR"/radio
